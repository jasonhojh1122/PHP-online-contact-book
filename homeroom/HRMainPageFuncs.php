...
sideBar;
	...
	function systemWarn($conn,$week,$userID){
		$result = "SELECT * FROM sign_homeroom WHERE teacherID = $userID";
	...
	function signEnable($conn,$week,$userID){
		$result = "SELECT `$week` FROM sign_homeroom WHERE teacherID = $userID";
		$result = $conn-> query($result);
		$result->data_seek(0);
...
disable;
	...
	function UpdateSign($conn,$time,$week,$userID,$input){
		...
		if ($input == $sPass){
			if ($result==NULL){
				$sign = "UPDATE `sign_homeroom` SET `$week` = '$time' WHERE teacherID = $userID";
				$conn-> query($sign);
				echo "<script>alert('本週簽到成功 !');window.location.href='HRMainPage.php';</script>";
			}			
		}
		else{
			echo "<script>alert('驗證碼錯誤 !');window.location.href='HRMainPage.php';</script>";			
		}
	...
	function UpdateSignA($conn,$time,$week,$userID,$input){
		...
		if($input == $sPass){
			if ($result==NULL){
				$sign = "UPDATE `sign_homeroom` SET `$week` = '$time' WHERE teacherID = $userID";
				$conn-> query($sign);
				echo "<script>alert('此週補簽成功 !');window.location.href='HRAnnouncement.php?chosenWeek=$week';</script>";
			}			
		}
		else{
			echo "<script>alert('驗證碼錯誤 !');window.location.href='HRAnnouncement.php?chosenWeek=$week';</script>";			
		}
	}
